...
# -*- coding: utf-8 -*-
"""
Created on Sat Jul 11 18:45:43 2020
...
def equations_cubic(lp):
    ...
    return [1/(wavelenght/(2*m.sin(m.radians(two_theta[i]/2))))**2 - (h[i]**2 + k[i]**2 + l[i]**2)/a**2
            for i in range(len(two_theta))]

def equations_hexagonal(lp):
    a, c = lp
    return [1/(wavelenght/(2*m.sin(m.radians(two_theta[i]/2))))**2 - 4/3*(h[i]**2 + h[i]*k[i]+k[i]**2)/a**2 - l[i]**2/c**2
            for i in range(len(two_theta))]

def equations_rhombohedral(lp):
    a, alpha = lp
    return [1/(wavelenght/(2*m.sin(m.radians(two_theta[i]/2))))**2 - ((h[i]**2 + k[i]**2 + l[i]**2)*(m.sin(m.radians(alpha)))**2 + 2*(h[i]*k[i] + k[i]*l[i] + h[i]*l[i])*((m.cos(m.radians(alpha)))**2 - (m.cos(m.radians(alpha)))))/a**2/(1-3*(m.cos(m.radians(alpha)))**2+2*(m.cos(m.radians(alpha)))**3)
            for i in range(len(two_theta))]

def equations_tetragonal(lp):
    a, c = lp
    return [1/(wavelenght/(2*m.sin(m.radians(two_theta[i]/2))))**2 - (h[i]**2 + k[i]**2)/a**2 - l[i]**2/c**2
            for i in range(len(two_theta))]

def equations_orthorhombic(lp):
    a, b, c = lp
    return [1/(wavelenght/(2*m.sin(m.radians(two_theta[i]/2))))**2 - h[i]**2/a**2 - k[i]**2/b**2 - l[i]**2/c**2
            for i in range(len(two_theta))]
...
def equations_triclinic(lp):
    ...
    V = a*b*c*(1-m.cos(m.radians(alpha))**2 - m.cos(m.radians(beta))**2 - m.cos(m.radians(gamma))**2 + 2*(m.cos(m.radians(alpha))*m.cos(m.radians(beta))*m.cos(m.radians(gamma))))**0.5
    
    return [1/(wavelenght/(2*m.sin(m.radians(two_theta[i]/2))))**2 - 1/V**2*(s11*h[i]**2+s22*k[i]**2+s33*l[i]**2+2*s12*h[i]*k[i]+2*s23*k[i]*l[i]+2*s13*h[i]*l[i])
            for i in range(len(two_theta))]
...
def solve_equations(lattice_system):
    ...
    if lattice_system == 'Cubic':
        ...
        # solving triclinic
        a, b, c, alpha, beta, gamma =  fsolve(equations_triclinic, [a_start, b_start, c_start, alpha_start, beta_start, gamma_start])
...
number_of_peaks = {'Triclinic':6, 'Monoclinic':4, 'Orthorhombic':3, 'Tetragonal':2, 'Rhombohedral':2, 'Hexagonal':2, 'Cubic':1} 
...
while True:
    ...
    if event == 'Select': 
        ...
        layout =[     
                ...
                [sg.Text('2 Theta', size=(22, 1)), sg.Text('h', size=(10, 1)), sg.Text('k', size=(10, 1)), sg.Text('l', size=(10, 1))],
                ]
        for i in range(number_of_peaks[lattice_system]):
            layout.append([sg.In(size=(10, 1)),sg.Text('°', size=(5, 1)), sg.In(size=(10, 1)), sg.In(size=(10, 1)), sg.In(size=(10, 1))])
